...
import { UpgradeCategory } from '../systems/UpgradeSystem';

interface UpgradeVisual {
  category: UpgradeCategory;
  zIndex: number;
  render: (ctx: CanvasRenderingContext2D, houseX: number, houseY: number, time: number) => void;
}

export class HouseRenderer {
  private upgradeVisuals: Map<UpgradeCategory, UpgradeVisual>;
  ...
  private initializeUpgradeVisuals(): void {
  ...
  private drawRune(ctx: CanvasRenderingContext2D, x: number, y: number, size: number): void {
   ...
   */
  drawBaseHouse(ctx: CanvasRenderingContext2D, houseX: number, houseY: number): void {
    // House body
  ...
  drawUpgradeVisuals(
    ...
    const categories = new Set<UpgradeCategory>();
    for (const upgradeId of purchasedUpgrades) {
      ...
      if (parts.length >= 2) {
        ...
        const category = parts.slice(0, -1).join('_') as UpgradeCategory;
    ...
    const visualsToRender = Array.from(categories)
      ...
      .filter((v): v is UpgradeVisual => v !== undefined)
