...
class Game {
  ...
  private kiroImage: HTMLImageElement | null = null;
  private ghostTimeOffset: number = Math.random() * Math.PI * 2; // Random start position
  private upgradeButtons: Array<{ upgrade: Upgrade; x: number; y: number; width: number; height: number }> = [];
  ...
  start(): void {
    console.log('🎮 TypingKiro starting...');
    this.gameLoop.start();
  }
  ...
  private renderGame(): void {
    ...
    this.renderer.drawText(
      `Fright Points: ${state.frightPoints}`,
      20,
      50,
      Colors.PURPLE_300,
      20
  ...
  private drawFamilies(): void {
    ...
    for (const family of families) {
      ...
      if (family.scared) {
        ...
        this.renderer.drawText(
          remainingPart,
          family.position.x - 30 + typedWidth,
          wordY,
          Colors.FOG_100,
          20
        );
  ...
  private drawStickFigure(x: number, y: number, color: string): void {
    const ctx = this.renderer.getContext();
  ...
  private drawFloatingGhost(): void {
    const ctx = this.renderer.getContext();
