...
export class UpgradeSystem {
    getAvailableUpgrades(purchasedUpgrades) {
    ...
    canPurchase(upgradeId, frightPoints, purchasedUpgrades) {
    ...
    purchaseUpgrade(upgradeId, frightPoints, purchasedUpgrades) {
        const upgrade = this.getUpgradeById(upgradeId);
    ...
    applyUpgradeEffects(purchasedUpgrades) {
        const stats = {
            damageReduction: 0,
            spawnDelay: 0,
            pointMultiplier: 1.0,
    ...
    getTotalDifficultyIncrease(purchasedUpgrades) {
        let total = 0;
        for (const upgradeId of purchasedUpgrades) {
            const upgrade = this.getUpgradeById(upgradeId);
            if (upgrade) {
    ...
    getDamageReduction(purchasedUpgrades) {
        let reduction = 0;
        for (const upgradeId of purchasedUpgrades) {
            const upgrade = this.getUpgradeById(upgradeId);
    ...
    getPointMultiplier(purchasedUpgrades) {
        let multiplier = 1.0;
        for (const upgradeId of purchasedUpgrades) {
            const upgrade = this.getUpgradeById(upgradeId);
